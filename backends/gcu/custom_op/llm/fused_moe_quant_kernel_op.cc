...
std::vector<paddle::Tensor> FusedMoeQuantKernel(
    ...
    const paddle::optional<paddle::Tensor>& real_token_num,
    int64_t gs,
  ...
  phi::DenseTensor* real_token_num_tensor = &tmp;
  if (real_token_num.is_initialized()) {
    real_token_num_tensor =
        static_cast<phi::DenseTensor*>(real_token_num.get().impl().get());
  }
  ...
  auto real_token_num_tensor_aten =
      custom_kernel::CreateTopsatenTensor(*real_token_num_tensor);
...
PD_BUILD_OP(fused_moe_quant_kernel)
    .Inputs({
        ...
        paddle::Optional("real_token_num"),
    })
    .Outputs({"C_out"})
    .Attrs({
        "gs: int64_t",
