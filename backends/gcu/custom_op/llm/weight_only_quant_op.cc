...
std::vector<std::vector<int64_t>> WeightOnlyQuantInferShape(
    std::vector<int64_t> lhs_shape, std::vector<int64_t> rhs_shape) {
  lhs_shape.at(lhs_shape.size() - 1) = rhs_shape.at(0);
  return {lhs_shape};
}

std::vector<paddle::DataType> WeightOnlyQuantInferDtype(
    const paddle::DataType &lhs_dtype, const paddle::DataType &rhs_dtype) {
  return {lhs_dtype};
}

std::vector<paddle::Tensor> WeightOnlyQuantKernel(
    const paddle::Tensor &lhs,
    const paddle::Tensor &rhs,
    const paddle::optional<paddle::Tensor> &bias,
    const paddle::Tensor &scale,
    int group_size) {
  PADDLE_GCU_KERNEL_TRACE("weight_only_quant_kernel");
  VLOG(6) << "[CUSTOM_KERNEL] Custom Operator: weight_only_quant_kernel";
  ...
  std::string abstract_info =
      custom_kernel::GetAbstractInfo("topsatenLinearQuant",
                                     linear_out,
                                     lhs_tensor,
                                     rhs_tensor,
                                     bias_tensor,
  ...
  auto status = topsaten::topsatenLinearQuant(linear_out_aten,
                                              lhs_tensor_aten,
                                              rhs_tensor_aten,
                                              bias_tensor_aten,
                                              scale_tensor_aten,
...
PD_BUILD_OP(weight_only_quant_gcu)
    .Inputs({"lhs", "rhs", paddle::Optional("bias"), "scale"})
    .Outputs({"linear_out"})
    .Attrs({
        "group_size: int",
    })
    .SetKernelFn(PD_KERNEL(WeightOnlyQuantKernel))
    .SetInferShapeFn(PD_INFER_SHAPE(WeightOnlyQuantInferShape))
    .SetInferDtypeFn(PD_INFER_DTYPE(WeightOnlyQuantInferDtype));