...
class Expert(nn.Layer):
    ...
    def __init__(self, d_model, d_feedforward):
        super().__init__()
        self.fc1 = nn.Linear(d_model, d_feedforward * 2, bias_attr=False)  # *2 for SwiGLU
...
class TestFusedMoeConsistency(unittest.TestCase):
    ...
    # qweight.shape: [E, K, N] -> [E, N, K/2], scale: [E, K, N]
    def gcu_w4_quant_moe(self, qweight):

        # qweight.shape: [N/2, K] -> [N, K/2]
        def w4_rearrange_per_E(qweight):
            N, K = qweight.shape[0] * 2, qweight.shape[1]
            qweight = qweight.transpose([1, 0])
            shift_mask = paddle.to_tensor([0, 4], dtype='int8')
            qweight = paddle.bitwise_right_shift(paddle.unsqueeze(qweight, 2).expand((-1, -1, 2)), shift_mask).reshape((K, N))
            ...
            if K % 128 != 0:
                ...
                qweight = paddle.concat([qweight, padding], dim=0)
            ...
            try:
                ...
                rqweight |= paddle.bitwise_left_shift(qweight[shifts[1::2].reshape((-1,))], shift_scalar_tensor)
        ...
        # qweight.shape: [E, K, N]
        E, _, N = qweight.shape[0], qweight.shape[1], qweight.shape[2]
        self.group_size = 128

        # qweight.shape: [N/2, K], scales.shape: [K/gs, N]
        rqweight, scales = weight_quantize(qweight[0], "weight_only_int4", arch=80, group_size=self.group_size)
        rqweight = w4_rearrange_per_E(rqweight)

        for i in range(1, E):

            qw_i, scales_i = weight_quantize(qweight[i], "weight_only_int4", arch=80, group_size=self.group_size)
            qw_i = w4_rearrange_per_E(qw_i)

            rqweight = paddle.concat([rqweight, qw_i], axis=0)
            scales = paddle.concat([scales, scales_i], axis=0)

        return rqweight.reshape((E, N, -1)), scales.reshape((E, -1, N))
    ...
    def prepare_data(self):
        ...
        self.w0, self.scale0 = self.gcu_w4_quant_moe(self.w0)
        self.zeros0 = self.scale0 * 8
        ...
        self.w1, self.scale1 = self.gcu_w4_quant_moe(self.w1)
        self.zeros1 = self.scale1 * 8
    ...
    def fused_moe_forward(self, x: paddle.Tensor) -> paddle.Tensor:
            ...
            gcu_ops.topk_softmax(topk_weights, topk_indices, token_expert_indices, gate_out, norm_topk_prob = False)
            ...
            sorted_token_ids, expert_ids, num_tokens_post_pad = gcu_ops.moe_align_block_size(
                sorted_token_ids,
                expert_ids,
                num_tokens_post_pad,
                topk_indices,
    ...
    def test_consistency(self):
        ...
        # Compute outputs
        base_out = self.baseline_forward(self.x).to(paddle.float32).cpu()
        fused_out = self.fused_moe_forward(self.x).to(paddle.float32).cpu()

        N = base_out.shape[-1]
        B = base_out.shape[0] * base_out.shape[1]

        cosine_similarity = paddle.nn.functional.cosine_similarity(fused_out.reshape((-1, N)), base_out.reshape((-1, N)), axis=1)

        self.assertTrue(paddle.all(cosine_similarity.to(paddle.float32).cpu() > 0.98) ,
            msg=f"test_GCUMOEQuant fail: quant_result:\n{fused_out} vs unquant_result:\n{base_out}, cosine_similarity: {cosine_similarity}."
        )
